...
public class Carservice {
	...
	public void ex3() {
		...
		for(Car c: carList) {
			...
			if(c instanceof Truck) {
				//c 가 참조하는 변수가 Truck 객체인 경우
				System.out.println("최대 적재 하중 : "+ ((Truck)c).getWeight()+"t");
				// 만약 다운 캐스팅이 잘못될 경우
				// ClassCastException이 발생한다!
				// ex) LightCar 객체를 Truck 참조변ㄴ수 참조하려고 할 때
				
				// 해결 방법 : instanceof 를 통해 참조하는 객체의 자료형을 판단하여 
				// 적절한 형식으로 다운 캐스팅을 진행
				...
				//c 가 참조하는 객체가 Truck, LightCar가 아닐 때 
	...
	public void printCar(Car c) {
		...
		//동적 바인딩 -> 오버로딩이 되야하고 부모 객체를 참조하고 있어야만 하는가?
	}
