...
//			System.out.println(s.toString());
	...
	private void selectGrade() {
		 ...
		 * 조회할 학년을 입력하세요 : 3
		 * 
		 * ***********조회한 학년이 있을경우*********** [3학년 조회 결과] 3학년 5반 2번 홍길동 3학년 2반 1번 이미영
		 * 
		 * ***********조회한 학년이 없을경우********** [3 학년 학생이 존재하지 않습니다]
		 */
		...
		if (list.isEmpty())
			...
			System.out.printf("[%d학년 조회 결과]", input);
			for (Student s : list)
				System.out.printf("\n %d학년 %d반 %d번 %s ", s.getGrade(), s.getClassRoom(), s.getNumber(), s.getName());
		}
	...
	private void selectGender() {
		 ...
		 * 
		 **** 잘못 입력하는 경우 **** 조회할 성별을 입력하세요 (M/F) : a
		 * 
		 * [M또는 F만 입력 해주세요.}
		 * 
		 * 조회할 성별을 입력하세요(M/F) : (정상 입력이 될 때 까지 무한 반복) 정상 입력인 경우 **** 조회할 성별을 입력하세요 (M/F)
		 * : F [여햑생 목록] 2학년 7반 12번 김갑순(F), 2학년 3반 8번 이미영(F), 3학년 1반 2번 김샛별(F),
		 * 
		 * 
		 */
