...
public class ForEx {
    ...
    //두 정수를 입력 받아
    	...
    	System.out.printf("%d부터 %d까지 모든 정수의 합 : %d",num, num1 ,sum);
    ...
    public void ex6() {
    	...
    	for(double i = 10; i <= 20;i += 0.5) {
    		// 초기식으 ㄹint형으로 지정하게 되는 경우 무한루프에 빠지게 된다.
    		// 왜? i += 0.5 수행 시 10.5 가 아닌 
    		//       10(int형으로)으로 형변환되어 i에 대입되기 때문에
    		//        i가 계속 10을 유지한다
 ...
 // for문 응용 사용법 2
    public void ex8() {
    	...
    	int sum = 0; //합게를 저장할 변수 선언 및 0으로 초기화
    ...
    public void ex14() {
    	...
    	if(input<2 || input>9){ System.out.print("잘못 입력하셨습니다");
