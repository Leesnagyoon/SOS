...
public class EmployeeView {
     ...
     public void displayMenu() {
    	 ...
    	 do {
    		 try {
    			...
    			//사번, 이름 , 부서명 , 직급명, 전화번호 N행 조회 (사번 오름 차순)
    ...
    private void selectName() {
    	...
    	for(Employee emp : empList) {
    		...
    		System.out.println("[글자가 이름에 포함된 사원 조회 중 예외발생]");
    		e.printStackTrace();
    	}
    }
    /** 급여 범위 조회
    ...
    private void selectSalary() {
        ...
        try {
			...
			if(empList.isEmpty()) {
				System.out.println("[입력된 급여 범위 내 조회된 사원이 없습니다.]");
				return;
			}
    ...
    private void retireEmployee() {
    	...
    	try {
			...
			System.out.println("[퇴사 처리 진행 중 예외 발생]");
			e.printStackTrace();
