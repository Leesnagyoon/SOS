...
public static void main(String[] args) {
	...
	// -> DBeaver 의 계정 접속 방법 추가와 유사함
	
	// 왜 Connection 은 인터페이스일까??
	...
	try {
		...
		String dbName = ":ORCL"; // DB 이름 (비슷한 단오로 SID)
		// 19c 버전 :  :ORCL
		// 18c 또는 강의장 DB :  :XE
		
		String user = "kh_lsyun"; // 아이디
		
		String pw = "oracle_lsyun0714";// 비밀번호
...
//	  System.out.println(conn);
	 // -> 정상적으로 작성한 경우 conn 객체의 주소가 출력
	  //   잘못 작성한 경우 SQLException 예외 발생
	  
	  // 3. SQL 작성
	  // ** Java에서 작성되는 SQL은 마지막에 세미콜론(;) 작성하지 않는다 **
	  ...
	  // executeQuery() : SELECT 문을 수행하는 메서드 
	  //                  결과로 ResultSet을 반환
	  ...
	  while(rs.next()) {
		  ...
		  // [Java]             [DB]
		  // String           CHAR,VARVHAR2
		  // int,long         NUMBER('정수'만)
		  // float,double     NUMBER('실수'만)
		  // java.sql.Date    DATE
		  ...
		  // 현재 커서가 접근한 행의 "SARARY" 컬럼의 값을 얻어와 
		  // int 반환
