...
SELECT 'AABAACAABBAA',INSTR('AABAACAABBAA','B',5)FROM DUAL; --9
                      --     123456789
-- 문자열 5번째 부터 검색하여 두번째로 찾는 B의 위치 조회
...
SELECT 123.456 ,
    ...
    ROUND(123.456,-1), -- 소수점 -1번째 -> 정수 1의 자리에서 반올림
    ROUND(123.456,-2) -- 소수점 -2 번째 > 정수 10의 자리에서 반올림
FROM DUAL;
...
SELECT '약'
||MONTHS_BETWEEN('2023/07/14','2023/02/06')
||'개월'훈련기간
FROM DUAL;
...
FROM DUAL;
SELECT SYSDATE ,TO_CHAR(SYSDATE,'YY-MM-DD DY AM HH:MI:SS')
FROM DUAL;
...
SELECT TO_DATE('2023년03월07일','YYYY"년"MM"월"DD"일"')
FROM DUAL; 
...
/* 날짜 패턴 Y, R의 차이점*/
-- 년도를 짧게 해석하는 경우 
-- 50미만 : Y,R 둘다 앞부분에 현재 세기 (21세기)적용
-- 50이상 : Y == 현재 세기(2000년대)
--         R == 이전 세기(1900년대)
...
FROM EMPLOYEE;-- 중복 제외하고 카운트

--EMPLOYEE 테이블의 남자 사원수 , 여자 사원 수 조회
