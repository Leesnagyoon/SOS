 ...
 * (기본값)NOFORCE : 서브쿼리에 사용된 테이블이 존재해야만 뷰 생성
 *  
 ...
 *                   단 , 생성 시퀀스가 생성 되자마자 호출할 경우 오류 발생.
 *                   == 마지막으로 호출한 NEXVAL값을 반환
 * */
...
--다시 NEXTVAL 호출
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL; -- 5씩 증가함
...
--SEQ_TEST_NO의 증가값을 1, 최대값 155 수정
...
-- 시퀀스는 START WITH 를 변경하고 싶을 때 삭제 후 다시 생성해야 한다.
 ...
 *  **인덱스가 자동 생성되는 경우 **
 * 
 *  ->PK 또는 UNIQUE 제약 조건이 설정된 컬럼에 대해 
...
WHERE TEST_ID = 'TEST500000'; -- 20/14/20/20
--인덱스 사용 O 조회 
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = 500000; -- 1/1/1ms
...
--인덱스 생성 후 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; -- 0/0/0/1ms
...
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; --삭제 후 22/20/16ms
