...
-- DUAL(DDmmy tAbLe) : 가짜 테이블(실제 테이블이 아닌 임시 테이블 용도)
...
/*해석순서*/
/*2*/SELECT EMP_ID , EMP_NAME, SALARY , DEPT_CODE
/*1*/FROM EMPLOYEE
/*3*/WHERE SALARY > 3000000;
-- 단 SALARY 값이 300만 초과
...
-- EMPLOYEE 테이블에서
-- 이메일에 @ 앞글자가 5글자인 사원의 사번,이름,이메일 조회
SELECT EMP_ID ,EMP_NAME ,EMAIL 
FROM EMPLOYEE
WHERE EMAIL LIKE '_____@%';

-- EMPLOYEE 테이블에서
-- 이메일에 _ 앞글자가 3글자인 사원의 사번,이름,이메일 조회
SELECT EMP_ID ,EMP_NAME ,EMAIL 
FROM EMPLOYEE
...
FROM  EMPLOYEE
WHERE 연봉>= 5000000;
-- 오류 발생(SELECT 절 해석이 안된 상태에서 별칭을 WHERE절 작성)
