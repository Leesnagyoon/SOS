...
CREATE TABLE MEMBER(
   ...
   MEMBER_SSN  CHAR(14), -- 123456-1234567(항상 14글자)
   ENROLL_DATE DATE DEFAULT SYSDATE --가입일
   
   --DEFAULT(기본값) : INSERT/UPDATE 시 
   --                해당 컬럼의 값을 작성하지 않거나 
   --                DEFAULT로 작성하면
   --                DEFAULT 오른쪽에 작성된 값이 컬럼에 반영 
);
...
--> 문제 해결 방법 : CHAR or VARCHAR2 타입으로 컬럼 타입으로 지정
...
--    USER_NO NUMBER PRIMARY KEY ,-- 컬럼 레벨(제약조건명X)
--    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY,--컬럼 레벨(제약 조건명O)
      USER_NO NUMBER,
...
--3) 테이블의 형태(컬럼명,데이터 파일)만 복사 (컬럼 값 X)
...
FOREIGN KEY(LOCATION_ID) REFERENCES LOCATION;-- 테이블명만 작성하면 자동으로 PK연결
