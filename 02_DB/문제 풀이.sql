...
-- 단 ,입사일 -오늘의 별칭은 "근무일수 1",
-- 오늘 - 입사일의 별칭은 "근무일수2"로 하고
-- 모두 정수(내림)처리 , 양수 처리 되도록 처리

SELECT EMP_NAME 사원명,
FLOOR(ABS(HIRE_DATE -SYSDATE)) "근무일수 1",
FLOOR (ABS(SYSDATE -HIRE_DATE)) "근무일수 2" 
FROM EMPLOYEE;
...
--14칸의 공간을 만들고 앞자리부터 8자리까지 EMP_NO를 채우고 남은 공간을 '*'로 채워라
FROM EMPLOYEE;
...
--6. EMPLOYEE 테이블에서 부서코드가 D5,D9 인 직원들 중에 2004년도에 입사한 사원의
-- 사번,사원명,부서코드 , 입사일 조회
...
SELECT EMP_NAME ,HIRE_DATE ,(LAST_DAY(HIRE_DATE)-HIRE_DATE)+1"입사한 달의 근무일수 조회"
FROM EMPLOYEE;
...
--TRUNC((SYSDATE- TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD'))/365)"만 나이"
