--1. 춘 기술대학교의 학과 이름과 게열을 표시하시오.단,출력 헤더는 "학과 명","계열"으로 표시하도록 한다.
SELECT DEPARTMENT_NAME "학과 명" ,CATEGORY "계열"
FROM TB_DEPARTMENT;

--2. 학과의 학과 정원을 다음과 같은 형태로 화면에 출력한다.
SELECT DEPARTMENT_NAME"학과 명",'의 정원은'"1", CAPACITY"정원",'명 입니다' AS "학과별 정원" 
FROM TB_DEPARTMENT;
...
--   그 대상자들의 학번이 다음과 같을 떄 대상자들을 찾는 적절한 SQL 구문을 작성하시오.
...
--1. 영어영문학과(학과코드 002) 학생들의 학번과 이름, 입학 년도를 입학 년도가 빠른순으로 표시하는 SQL문장을 작성하시오
--   (단 , 헤더는 "학번" , "이름","입학년도" 가 표시 되도록한다.)
...
--   (교수중 2000년 이후 출생자는 없으며 출력 헤더는 "교수 이름","나이"로 한다.나이는 '만'으로 계산한다)

SELECT PROFESSOR_NAME "교수 이름",
SUBSTR(TO_NUMBER(TO_CHAR(SYSDATE,'YYYY'))-TO_NUMBER(SUBSTR(PROFESSOR_SSN,1,2)),3,2)"나이"
FROM TB_PROFESSOR
...
--   이 때,19살에 입학하면 재수하지 않은 것이라고 간주한다
SELECT STUDENT_NO , STUDENT_NAME,
MONTHS_BETWEEN(ENTRANCE_DATE,TO_DATE(SUBSTR(STUDENT_SSN,1,4),'RRMM'))/12
FROM TB_STUDENT;

--6. 2020년 크리스마스는 무슨 요일인가?
SELECT TO_CHAR(TO_DATE('2020/12/25'),'YYYY"년"MM"월"DD"일 "DAY')
FROM DUAL;
...
--  2099년 10월 11 일 , 2049년 10월 11일 
--  1999년 10월 11 일 , 2049년 10월 11일
...
--9. 학번이 A517178 인 한아름 학생의 학점 총평점을 구하는 SQL 문을 작성하시오.
-- 단, 이때 출력 화면의 헤더는 "평점"이라고 찍히게 하고, 점수는 반올림하여 소수점 이하 한자리 까지만 표시한다.
...
--11.지도 교수를 배정받지 못한 학생의 수는 몇 명 정도되는지 알아내는 SQL문을 작성하시오.
SELECT COUNT(*)
FROM TB_STUDENT
WHERE COACH_PROFESSOR_NO IS NULL;

--12. 학번이 A112113인 김고운 학생의 년도 별 총점을 구하는 SQL문을 작성하시오.
--    단, 이 때 출력 화면의 헤더는 "년도","년도 별 총점"이라고 찍히게 하고,점수는 반올림하여 소수점 이하 한 자리까지만 표시한다.
SELECT SUBSTR(TERM_NO,1,4)"년도" ,ROUND(AVG(POINT),1)"년도 별 총점"
FROM TB_GRADE
...
--1. 학생이름과 주소지를 표시하시오.
--   단, 출력 헤더는 "학생 이름","주소지"로 하고, 정렬은 이름으로 오름차순 표시하도록 하라.
...
--    TB_DEPARTMENT ,TB_STUDENT,TB_PROFESSOR
SELECT DEPARTMENT_NAME "학과이름",STUDENT_NAME,PROFESSOR_NAME 
FROM TB_STUDENT
NATURAL JOIN TB_DEPARTMENT
...
--    (기존 워크북 PDF에 나타난 조회 결과는 DB 버전이 낮아 현재와 조회 방식이 다름.
--    결과 행의 수만 동일하게 조회하자)
--TB_CLASS , TB_DEPARTMENT(CATEGORY)
SELECT CLASS_NAME,DEPARTMENT_NAME
FROM TB_CLASS
...
--15. 휴학생이 아닌 학생 중 평점이 4.0 이상인 학생을 찾아 그 학생의 학번, 이름, 학과, 평점을 출력하는 SQL문을 작성하시오
--TB_STUDENT ,TB_DEPARTMENT ,TB_GRADE
...
-- TB_DEPARTMENT , TB_GRADE , TB_CLASS
SELECT CA.CLASS_NO,CA.CLASS_NAME , TRUNC(AVG(POINT),8) 
FROM TB_DEPARTMENT DE
JOIN TB_CLASS CA ON (DE.DEPARTMENT_NO = CA.DEPARTMENT_NO)
JOIN TB_GRADE GR ON (GR.CLASS_NO = CA.CLASS_NO)
WHERE DEPARTMENT_NAME = '환경조경학과'
AND CA.CLASS_TYPE = '전공선택'
GROUP BY CA.CLASS_NO , CA.CLASS_NAME
ORDER BY CA.CLASS_NO ;
...
FROM (
		SELECT STUDENT_NO 
				,AVG(POINT) 
				,RANK() OVER(ORDER BY AVG(POINT)DESC) AS 순위 
		FROM TB_DEPARTMENT DE
		JOIN TB_CLASS CL ON (DE.DEPARTMENT_NO=CL.DEPARTMENT_NO)
		JOIN TB_GRADE GR ON (GR.CLASS_NO=CL.CLASS_NO)
		WHERE DEPARTMENT_NAME = '국어국문학과'
		GROUP BY STUDENT_NO		
	)Z
WHERE Z.순위 = 1
...
SELECT DE.DEPARTMENT_NAME AS "계열 학과명"
		,ROUND(AVG(GR.POINT),1) AS "전공평점"
FROM TB_DEPARTMENT DE
JOIN TB_CLASS CL ON (DE.DEPARTMENT_NO = CL.DEPARTMENT_NO)
JOIN TB_GRADE GR ON (GR.CLASS_NO = CL.CLASS_NO)
WHERE DE.CATEGORY = (SELECT DE1.CATEGORY 
                  FROM TB_DEPARTMENT DE1
                  WHERE DE1.DEPARTMENT_NAME = '환경조경학과'
)
AND CL.CLASS_TYPE = '전공선택'
