...
function testFn(){
    ...
    console.log("if문 종류 후 (if)num5 : "+num5);
    console.log("if문 종류 후 (if)num6 : "+num6);
}

//함수 호출
testFn();

        // num3 is not defined(정의 되지 않거나 접근 불가능)
    // console.log("함수 내부에서 선언된 (함)num3"+num3); 

    //JS 코드 중간에 에러가 발생하면
    //같은 파일/함수 내에= 에러 이후 부분 코드가 해석되지 않는다
    console.log("함수 내부에서 선언된 (전)num4"+num4);
    ...
    const temp = 999;
    // temp = 1000; signment to constant variable. 에러
    ...
    testFn2(); // 함수 호출
    // console.log("함수 내부에 선언한 let2"+let2); 에러 let2 is not defined
    ...
    function typeTest(){
        ...
        const gender = 'M';
        box.innerHTML += "<br>gendr : "+ gender + "/"+ typeof gender;
