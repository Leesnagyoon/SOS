...
public class ArrayEx1 {
	...
	public void ex1() {
		// 배열 선언
		int[] arr;
		//- int[]을 참조할 공간을 할당하고 이를 arr이라고 부르겠다.
	...
	public void ex2() {
	    ...
	    // -> 컴파일러가 0(자료형의 기본값)으로 초기화 하였음을 확인
	...
	public void ex4() {
		//오늘의 점심 메뉴 뽑기
		String[] meauArray = {"김밥+라면","서브웨이","KFC","맘스터치","순대국","뼈해장국","닭갈비","마라탕"
		,"우육면","파스타","샐러드"};
		// 0부터 배열 길이의 범위 내에서 난수 발생
		int index = (int)(Math.random() * meauArray.length);
		
		System.out.println("오늘의 점심 메뉴 !! : "+meauArray[index]);
	...
	public void ex5() {
		...
		// 배열  인덱스  범위 초과 예외(에러)발생 : 인덱스 5번은 길이 5짜리 배열의 범위를 넘어섰다
	 ...
	 public void ex6() {
		 ...
		 for(int i =0;i<scoreArr.length;i++) {
			...
			if(scoreArr[i]<min) {
				min = scoreArr[i];
				//기존 min(최저값)보다 현재 배열 요소의 값이 더 작은 경우 
				minIndex = i; // 최저점 인덱스
			}
		 }
		
		 System.out.println("합계 : "+sum);
		 System.out.println("\n평균 : "+(sum/(double)scoreArr.length));
...
//				 2) flag 변수를 이용하는 방법
		 ...
		 for(int i = 0; i<arr.length;i++) {
			 ...
			 if(input == arr[i]) {//입력값과 일치하는 값이 존재하는 경우
				 index = i;
				 flag = false;
				 break;//일치하는 값을 찾은 경우 더 이상 반복하지 않게끔(성능 개선)
			 }
		 }
		 if(flag) {
			 System.out.println("존재하지 않습니다.");
